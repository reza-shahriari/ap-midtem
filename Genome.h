...
public:
    ...
    string create_second_DNA_from_first_DNA(string input_DNA1)
    {
        string make_DNA = input_DNA1;
        for (int i = 0; i < input_DNA1.length(); i++)
        {
            make_DNA[i] = Calculate_completed(input_DNA1[i]);
        }
        return make_DNA;
    }
    ...
    void create_reverse_mutation(string s1)
    {
        ...
        DNA[0] = RNA;
        DNA[1] = create_second_DNA_from_first_DNA(DNA[0]);
    }
};
